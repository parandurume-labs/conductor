# duru-conductor — Project Brief for AI

> 이 문서는 생성형 AI가 `duru-conductor` 프로젝트를 처음부터 만들 수 있도록 작성된 설계 문서입니다.
...
- `npx add-skill parandurume-labs/duru-conductor` 한 줄로 설치 가능
...
### 왜 duru-conductor인가
기존 Agent Skills(예: Vercel의 `react-best-practices`)는 단일 기술 도메인에 집중한다. `duru-conductor`는 여기서 한 발 더 나아가:
...
### `skills/duru-conductor/references/AGENT-TEAMS.md`
...
- 프로젝트명: `duru-conductor — AI Project Orchestration Agent Skill`
...
- 설치 명령: `npx add-skill parandurume-labs/duru-conductor`
- 스킬 목록 표
- 빠른 사용 예시 (한국어 포함)
...
□ skills/duru-conductor/SKILL.md — name: duru-conductor (폴더명 일치)
